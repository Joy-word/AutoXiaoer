...
- 🔧 **MCP 支持**：内置高德地图 MCP，并支持添加、测试、启停和管理自定义 MCP 服务器
...
- ✅ **微信远程控制（ClawBot）**：通过微信扫码连接，远程发送指令并接收任务执行反馈，并且支持查询详情与追加
...
- ✅ **数据迁移**：支持导入和导出人设、提示词、定时任务、MCP 配置等数据，方便在不同手机间迁移（注：API Key 等敏感信息不会被导出）

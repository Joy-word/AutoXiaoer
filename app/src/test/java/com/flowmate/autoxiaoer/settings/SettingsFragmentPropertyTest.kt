...
class SettingsFragmentPropertyTest :
    StringSpec({
        ...
        "phone agent config values should round-trip correctly" {
            checkAll(
                100,
                Arb.int(0, 1000),
                Arb.double(0.0, 10.0),
                ...
                val savedConfig =
                    SimulatedPhoneAgentConfig(
                        maxSteps = maxSteps,
                        screenshotDelayMs = screenshotDelayMs,
                        language = if (maxSteps % 2 == 0) "cn" else "en",
                    )

                val loadedConfig = simulatePersistAndLoadPhoneAgent(savedConfig)
...
data class SimulatedPhoneAgentConfig(val maxSteps: Int, val screenshotDelayMs: Long, val language: String)
...
/**
 * Simulates persisting and loading a PhoneAgent config.
 */
private fun simulatePersistAndLoadPhoneAgent(config: SimulatedPhoneAgentConfig): SimulatedPhoneAgentConfig = SimulatedPhoneAgentConfig(

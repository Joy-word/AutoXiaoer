...
class McpConnection(
    ...
    suspend fun connect() {
        ...
        val mcpClient = Client(
            clientInfo = Implementation(name = "autoxiaoer", version = "1.0", title = null),
    ...
    override suspend fun callTool(serverToolName: String, argumentsJson: String): String {
        ...
        val result = withTimeout(config.callTimeoutMs) {
            mcpClient.callTool(
                CallToolRequest(params = CallToolRequestParams(name = serverToolName, arguments = parsedArgs)),
            )
    ...
    private fun formatResult(result: CallToolResult): String {
